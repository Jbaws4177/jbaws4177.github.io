# Japan-Central Asia Young Professionals — Project Spec & Handoff
...
A bilingual (English / 日本語) marketing site for **Japan-Central Asia Young
Professionals (JCAP)** — a community connecting young people in Japan with the
Central Asian community living in Japan, through **networking events,
...
changed from Africa to Central Asia.

- **Audience:** open to *all* Central Asian countries, and to anyone in Japan
  interested in Central Asian culture, language, business, or exchange.
  Explicitly **not students-only** — young professionals, grads, entrepreneurs,
  and Central Asian residents/workers in Japan are all included.
- **Status:** pre-launch. Founded 2026. First event **"Japan–Central Asia
...
```
astro.config.mjs       ← site = ryumakojima.github.io, base = '/jcap'
src/
...
### Base path (important)
The site is a **GitHub Pages project site**, served from **`/jcap/`**. `base` is
...
- Palette: gold `#c5a059`, dark green-charcoal `#1e2b26`, sand `#f4edd8`,
  ...
  here if JCAP wants its own identity.)
...
- Open Graph + Twitter card (image = `/jcap/og.jpg`).
- JSON-LD `Organization` + `WebSite` + `WebPage` graph.
- `@astrojs/sitemap` (i18n-aware) → `/jcap/sitemap-index.xml`; `robots.txt` points to it.
...
- Repo: `ryumakojima/jcap`. `.github/workflows/deploy.yml` builds with
  `withastro/action` and deploys via `actions/deploy-pages` on push to
  `main`/`master`.
- Enable Pages → Source: **GitHub Actions** in repo settings.
- Served at `ryumakojima.github.io/jcap/` (project subpath). Base handling is
...
- **No fabricated data.** Unlike JAYP, this site intentionally ships with NO
  ...
  placeholder reach numbers — because JCAP is a fresh org. Add real, sourced
  data as it exists; don't invent citations.
- **Design colours are inherited from JAYP** (warm gold/sand). Consider a distinct
  JCAP palette in `global.css` once branding is decided.
...
2. Re-themed Africa → Central Asia across all EN/JA pages, nav/footer, SEO
   metadata, and JSON-LD; renamed to **Japan-Central Asia Young Professionals**.
3. Added a project **subpath deploy** (`base: '/jcap'`) with a `withBase()`
...
- A distinct JCAP colour palette, if desired.
